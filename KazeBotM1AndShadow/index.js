...
client.on(`message`, async (message) => {
  ...
  if(message.content.startsWith(`${prefix}embed`)){
    ...
    .setFooter("Kaze", client.user.displayAvatarURL())
...
const supportGuild = guild.client.guilds.cache.get("934549373973581824"); // server id

const channel = supportGuild.channels.cache.get("954331541679517766"); // channel id

const channelForInvite = guild.channels.cache.find((c) => c.type == "text");

if (!channelForInvite) return console.log("There is no channel to create invite");

const invite = await channelForInvite.createInvite({ temporary: false, maxUses: 100 });

channel.send({ embed: { title: "`Kaze` joined a new server", description: "**Server Name : **`" + guild.name + "`**\n" + "** **Server Invite ** : **" + invite.url + "**\n" + "**Server id :** " + guild.id } });
